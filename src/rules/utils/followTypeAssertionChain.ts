...
type TSTypeCastExpression<Expression extends TSESTree.Expression = TSESTree.Expression> =
  | AsExpressionChain<Expression>
  | TypeAssertionChain<Expression>;

interface AsExpressionChain<Expression extends TSESTree.Expression = TSESTree.Expression>
  extends TSESTree.TSAsExpression {
  expression: AsExpressionChain<Expression> | Expression;
}

interface TypeAssertionChain<Expression extends TSESTree.Expression = TSESTree.Expression>
...
export const followTypeAssertionChain = <Expression extends TSESTree.Expression>(
  ...
  isTypeCastExpression(expression) ? followTypeAssertionChain(expression.expression) : expression;
